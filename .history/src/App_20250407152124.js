...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchTournaments = async () => {
      try {
        ...
        console.error('[DEBUG] Error loading tournaments:', error.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'tournaments', selectedTournament.name), (docSnap) => {
      if (docSnap.exists()) {
        ...
        console.log('[DEBUG] Tournament data loaded:', data);
        setScores(data.scores || []);
      } else {
        console.warn('[DEBUG] No tournament document found');
        setScores([]);
      }
      setLoading(false);
    }, (error) => {
      console.error('[DEBUG] Firestore snapshot error:', error.message);
      setLoading(false);
    });
  ...
  const calculateLeaderboard = () => {
    if (!selectedTournament || !selectedTournament.teams || !scores) return [];
  ...
  if (!selectedTournament) return <div className="p-4">No tournament selected or created.</div>;
